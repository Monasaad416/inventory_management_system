                                            ...
                                            @foreach ($shareholders as $shareholder )
                                                <tr>
                                                   ...
                                                   {{ $shareholder->user->name }}
                                                    ...
                                                    <td>
                                                        <a href="{{route('shareholders.edit',$shareholder->id)}}" class="btn btn-info btn-sm" role="button" aria-pressed="true" title="تحديث بيانات المساهم"><i class="fa fa-edit text-white" aria-hidden="true"></i></a>
                                                    </td>

                                                    <td>
                                                        <button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#delete_shareholders{{ $shareholder->id }}" title="حذف المساهم"><i class="fa fa-trash"></i></button></td>
                                                                       <!-- Delete Modal -->
                                                        <form action="{{route('shareholders.destroy',$shareholder)}}" method="POST">
                                                            <div class="modal fade" id="delete_shareholders{{$shareholder->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                                ...
                                                                <div class="modal-content">
                                                                    ...
                                                                    <div class="modal-body">
                                                                        <p>هل انت متأكد من حذف المساهم  {{$shareholder->name}}</p>

                                                                            @csrf
                                                                            {{method_field('delete')}}
                                                                            <input type="hidden" value="{{$shareholder->id}}" name="id">
