                        ...
                        <form action="{{ route('suppliers_expenses.update',$supplierExpense) }}" method="post">
                           ...
                           <div class="row">
                                ...
                                <div class="col">
                                    ...
                                    <input type="number"  step="any" name="amount" min="0" class="form-control" value="{{ old('amount', $supplierExpense->amount) }}">
                                </div>
                                @error('amount')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                                    ...
                                    <select name="supplier_id"  class="form-control" >
                                        <option> ---إختار المورد---<option>
                                            @foreach (App\Models\Supplier::all() as $supplier )
                                                <option value={{ $supplier->id }}  {{ $supplierExpense->supplier_id == $supplier->id ? 'selected' : '' }}>{{ $supplier->name }}</option>
                                            @endforeach
                                    </select>
                                </div>
                                </div>
                            ...
                            <input type="hidden" name="supplierExpense_id" value="{{ $supplierExpense->id }}">
